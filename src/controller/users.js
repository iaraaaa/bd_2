...
export const addMateria = async (req, res) => {
  try {
    ...
    const { nombre } = req.body; // esto viene de la petición

    // validar con mi función
    const materiaExist = await userExist(cnn, "materia", "nombre", nombre);

    if (materiaExist) {
      // existe la materia en la bd
      return res.json({ message: "ya existe la materia" });
    } else {
      // creamos el query, usamos ? para prevenir inyección sql
      const [row] = await cnn.query(
        "INSERT INTO materia (nombre) VALUES (?)",
        [nombre]
      );

      // comprobar si se insertó en la bd
      if (row.affectedRows === 1) {
        // si se insertó
        return res.json({
          message: "se creó la materia con éxito",
          success: true,
        });
      } else {
        return res.status(500).json({ message: "no se creó la materia" });
      }
    }
  } catch (error) {
    console.log("addMateria", error.message);
    res.json({
      message: "no se pudo conectar con la base de datos",
      success: false,
    });
...
export const cursar = async (req, res) => {
  try {
    const { userId, materiaId } = req.body; // Suponiendo que recibes el ID del usuario y el ID de la materia desde el cuerpo de la solicitud
    const cnn = await connect();
    // Realiza la lógica para relacionar el usuario con la materia, por ejemplo, insertar una fila en una tabla intermedia que relacione usuarios y materias
    // Devuelve la respuesta apropiada
  } catch (error) {
    console.log("Error en la función cursar:", error);
...
export const getMateriasById = async (req, res) => {
  try {
    const { idAlumno } = req.params; // Suponiendo que recibes el ID del alumno como parámetro en la URL
    const cnn = await connect();
    // Realiza la consulta para obtener las materias que cursa el alumno con el ID proporcionado
    // Devuelve la respuesta apropiada
  } catch (error) {
    console.log("Error en la función getMateriasById:", error);
    res.status(500).json({ message: "Error interno del servidor" });
  }
};